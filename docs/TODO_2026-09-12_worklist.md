...
- [~] **1.5 重建身份库并验证** —— **S**（约 30 分钟机时）｜**从"可选"升级为"必需"**
  ...
  - **评测库已验证（msmt17 / 5 路 / 600s，2026-09-12）**：
    ...
    "塌缩→歧义→丢弃缓冲"的死循环已被打破。
  - 剩余动作：对**正式库**执行完整重建（备份 `outputs/lab_monitor.db` 后清空 identities
    与 identity_appearances，再让 22 路跑一轮，约 30 分钟）。评测库只有 5 路 10 分钟、
    6 个身份，样本量仍然偏小，正式重建后应复跑本脚本与 `probe_reid_separability.py` 复核。
...
| 批次 | 项数 | 已完成 | 阻塞关系 |
|---|---|---|---|
| 一 · 修特征层 | 7 | 5（1.1 / 1.2 / 1.4 / **1.6** / 1.7） | 剩 1.5 正式库重建（评测库已验证生效）、1.3 阈值标定 |
...
**下一步顺序**：1.5（正式库重建 + 复核）→ 1.3（标注集标定阈值与选权重）→ 批次二。
1.3 排在 1.5 之后是因为标定必须在**重建后的**特征空间上做：库内旧身份已被证明
不可离线修复，在它上面扫阈值毫无意义。注意 1.5 的评测数据显示中心化后 p95 = 0.7477
已**贴着**阈值 0.75 —— 说明 0.75 这个沿用多年的阈值大概率偏低，1.3 很可能会把它上调。
