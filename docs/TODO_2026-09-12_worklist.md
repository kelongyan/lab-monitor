...
- [ ] **1.3 建人工标注对 + 阈值标定（决定工作点与选哪份权重）** —— **M**
  ...
  - **1.6 之后本项才有意义**：在此之前扫出来的阈值只是"在一个已经失去判别力的特征空间里找最优切点"。
    现在中心化已把跨身份余弦从 +0.50 压到 -0.10，`0.75` 这个沿用多年的阈值几乎肯定偏低，
    需要重新定位（预计会往上走）。
  - 同时用它对比 msmt17 与 market1501 两份权重，给出"选哪份"的实测依据 ——
    `scripts/compare_reid_weights.py` 的 label-free 指标**不足以**区分二者。
...
- [ ] **1.8 身份归并（consolidation）：清理冷启动窗口造成的重复身份** —— **M** ｜**新增，1.5 复测发现**
  - 现象：重建后的 22 个身份里，`a093d0a6(rnd_01)` 与 `4652ffb5(rnd_05)`、
    `dd5845f8(rnd_02)`、`f5d0d193(rnd_11)`、`8f708fd8/bab050cd(rnd_17)`、`6cf46b48(rnd_22)`
    共 6 个身份的中心化特征**完全相同（sim=1.0000）**。这些相机全在同一条走廊链上
    （rnd_01→rnd_02 在拓扑里相邻），**极大概率是同一个人**。
  - 成因：中心化有冷启动护栏（有效特征 < 8 时**不启用**，这是必须的安全设计）。
    运行初 gallery 很小 → 中心化关闭 → 早期身份在**未中心化**的空间里各自注册；
    之后中心化生效，但**没有任何回溯归并机制** —— 已分开的身份永远不会合并。
  - 证据（`scripts/verify_rebuilt_identities.py`，生产语义 + 自检）：
    跨相机 226 对 p50=0.255 / p95=0.673；越阈 7 对全部集中在 `a093d0a6` 这一组 →
    **是"欠归并"而不是"误归并"** —— 方向与旧库（99.8% 互相误判）完全相反，说明修复有效。
  - 做法（候选，需 1.3 标定后定稿）：
    ① **定期归并**：后台任务周期性把中心化相似度 ≥ 阈值的身份合并，
       合并时保留 appearances 更多的一方、feature_bank 取并集去重；
    ② **中心预热**：用评测库或上一次运行的特征预置中心，让中心化从第一帧就生效；
    ③ 调低冷启动门槛（8 → 4），代价是早期中心更不准。
    推荐 **① + ②**：①解决已发生的分裂，②避免再次发生。
  - 验收：`verify_rebuilt_identities.py` 的跨相机越阈对数降为 0，
    且不出现"把两个真不同的人合到一起"（需 1.3 的标注集把关）。
...
| 批次 | 项数 | 已完成 | 阻塞关系 |
|---|---|---|---|
| 一 · 修特征层 | 8 | 6（1.1 / 1.2 / 1.4 / **1.6** / 1.7 / **1.5**） | 剩 1.3 阈值标定、1.8 身份归并 |
...
**下一步顺序**：1.8（身份归并，清理冷启动造成的重复）→ 1.3（标注集标定阈值与选权重）→ 批次二。
两者都需要 1.3 的标注集来定阈值与把关，所以实际推进是：
**先建标注集（1.3 前半），再实现 1.8 + 定阈值（1.3 后半）**。
注意 1.8 的归并必须由 1.3 的标注集把关 —— "中心化相似度 1.0"目前只证明了
"特征不可分"，还**没有**证明"是同一个人"，需要人工确认至少一组。
