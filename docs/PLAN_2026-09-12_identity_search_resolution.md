...
> ⚠️ **2026-09-12 实施期修正（重要）**：下文 0.1 与 0.2 的原始因果判断已被实验修正两轮。
> **第一次**：同一份 ImageNet 权重直接提原始特征并不塌缩（均值范数 0.796 / 余弦 p50 0.629），
> 库内那批 0.98 相似的特征是被 `update_appearance()` 的聚合加工过的 → 一度归因于"聚合方式"。
> **第二次**：实测"有界窗口均值"与 EMA 的塌缩**完全一样**（20 次观测 p50 0.7366 vs 0.7330），
> 聚合方式不是杠杆。真根因是**特征里的公共分量**（全体均值范数 0.80~0.83），
> 减去它之后跨身份余弦从 +0.50 掉到 **-0.10**、越阈比例从 21~46% 掉到 **0~7%**。
> 详见 §0.1a。原文判断保留在下方，以便对照——它记录了我们为什么会先怀疑权重。
...
```
① 公共分量未去除               →  全体特征均值范数 0.80~0.83（随机期望 ~0.1），
                                  跨身份余弦虚高到 +0.50、越阈 21.4%      ← 主因（已修）
② 特征聚合（EMA / 任何平均）    →  把残余的判别余量再挤掉一截，
                                  跨身份余弦升到 +0.73、越阈 46.4%        ← 放大器（保留现状）
③ ImageNet 分类权重            →  次要：三种权重的原始特征差异很小，
                                  且都不如"去公共分量"带来的改善大         ← 已换但非关键
...
### 0.1a 修公共分量（跨相机识别失效的真正根因）【阻塞项 · 实施期发现，优先级最高】

**证据链**（每一步都先做对照实验，两次推翻了看似合理的推断）：

_第一轮_：库内塌缩特征不是模型原始输出。
`scripts/compare_reid_weights.py` 在同一批 199 个真实 crop 上对比：imagenet 均值范数 0.7958 /
余弦 p50 0.6293、market1501 0.8148/0.6571、msmt17 0.7938/0.6234 —— **都远好于库内的 0.9754/0.984**。
⇒ 塌缩是流程加工出来的 → 一度判断"主因是聚合方式"。

_第二轮（推翻第一轮的修复方案）_：`scripts/diagnose_ema_collapse.py` 用**同一人**的时间连续样本
（同相机、间隔 0.4s、按 12 帧切块）对比两种聚合策略，跨身份余弦 p50：

| 聚合策略 | 5 次观测 | 20 次 | 50 次 | 200 次 |
|---|---|---|---|---|
| EMA(α=0.85) | 0.6531 | 0.7330 | 0.7281 | 0.7344 |
| 窗口均值 N=5 | 0.6871 | 0.7313 | 0.7126 | 0.7313 |
| 窗口均值 N=20 | 0.6840 | **0.7366** | 0.7384 | 0.7366 |

⇒ **两者完全一样**，换成有界窗口均值毫无收益。**聚合方式不是杠杆。**

_第三轮（找到真根因）_：所有单位特征都与一个**全局方向**高度共线 ——
全体特征均值向量范数 **0.80~0.83**（随机方向的期望只有 ~0.1）。减去它之后：
...
| **减公共分量（imagenet）** | **-0.1021** | **0.0%** |
| **减公共分量（msmt17）** | **-0.1319** | **0.0%** |
| **聚合后再减公共分量（msmt17 EMA 200 次）** | **-0.2823** | **3.6%** |

⇒ 聚合之所以"有害"，只是把这个残余的判别余量进一步挤掉。**病根是公共分量。**

**做法（已实施，改动面最小）**：中心化放在**匹配路径**上，不动 `update_appearance` ——
`IdentityStore._feature_center_locked()` 以全体已注册特征（主特征 + `feature_bank`）的均值
作为中心；`MatchContext` 把"已中心化的 gallery"与"所用中心"**原子打包**返回，
调用方必须用 `context.prepare(query)` 变换查询向量。

- **为什么必须打包**：gallery 与 query 若用了不同的中心（哪怕只是稍旧一点），两者就不在
  同一坐标系，**相似度全错却不报错**。拆成两个方法调用时，中间另一个线程注册新身份
  就会制造这种静默错配 —— 首版实现正是这个隐患，被单测抓出来。
- **两个护栏保证冷启动安全**：有效特征数 < 8 或中心范数 < 0.35 时**不启用**中心化，
  行为与改造前完全一致。
- `center_enabled` / `center_norm` 已外露到 `/api/metrics/reid` 与 `/api/system/metrics`。

**附带发现（很重要）**：**库内历史特征无法离线修复**。对生产库 45 个身份做离线中心化，
越阈比例只从 99.80% 降到 33.64%（主特征）/ 11.85%（含 bank），且**组内相似度变成 -0.130**
—— 说明这些身份的身份特异残差已被"混人 + 海量 EMA"彻底抹掉，中心化只能去掉公共分量、
救不回已丢失的信号。**所以 §1.5 的身份库重建不是可选优化，而是必需步骤。**

**验收**：重建后的库上跨身份余弦 p50 为负或接近 0、越阈比例个位数百分比；
`/api/metrics/reid` 的 `collapse_warnings` 长期为 0。
...
| `scripts/diagnose_ema_collapse.py` | 判定聚合方式是否为塌缩主因（EMA vs 有界窗口均值；`--center` 做公共分量对照；`--weights` 换权重） |
| `scripts/eval_identity_pipeline.py` | **有界端到端验证**：走真实链路跑指定秒数后停机，输出身份数/唯一 blob 数/中心化状态/原始与中心化两组余弦/匹配率/塌缩告警 |
