...
| 实测项 | 结果 | 判定 |
|---|---|---|
| 45 个身份之间，不同人的特征余弦相似度 | mean **0.950**、中位 **0.984**、max 1.000；990 对异人里 **988 对（99.8%）** 越过阈值 0.75 | **特征已退化，当前无法区分人** |
| 45 行身份记录的 `feature_blob` 唯一数 | 只有 **25** 个唯一值；7 组重复共 27 个身份，最严重 **7 个身份共享同一份特征** | **持久化缺陷，需定位** |
...
md5 完全相同只可能来自"同一个数组被写进多行"，不是特征相似。**根因尚未定位**——已排除的路径：`register()`/`register_if_new()` 都在锁内生成独立 `uuid4()[:8]` 且 `feature_bank=[feat_copy.copy()]`，`db.save_identity` 的 SQL 列序与占位符顺序一致。下一步排查方向：多线程下 `payload` 在锁内采集、**锁外**写库时 `_persist_full()` 兜底路径（`identity_store.py:507`）是否用了过期引用。

**做法**：

1. 复用 `scripts/diagnose_reid_degeneracy.py`（按 blob md5 分组 + 组内均值/残差诊断）。
2. 在 `register_if_new()` 返回前加一道护栏：若新身份主特征与 gallery 中任一主特征 cosine ≥ 0.999，打 `ERROR` 日志并附带两个 gid（不要静默通过）。
3. 补一条单测：并发注册 N 个互不相同的特征，断言 `feature_blob` md5 唯一数 == N。
